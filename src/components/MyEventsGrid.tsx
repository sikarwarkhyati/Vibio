...
import { Badge } from '../components/ui/badge';
import { Separator } from '../components/ui/separator';
import { Eye, Users, Calendar, MapPin, DollarSign } from 'lucide-react';
import { format } from 'date-fns';
...
const MyEventsGrid: React.FC<MyEventsGridProps> = ({ organizerId }) => {
  const { user } = useAuth();
  const { toast } = useToast();
  ...
  const fetchMyEvents = async () => {
    ...
    try {
      ...
      const transformed: MyEvent[] = (Array.isArray(fetched) ? fetched : []).map((ev: any) => ({
        ...ev,
        id: ev._id ?? ev.id ?? String(Math.random().toString(36).slice(2, 9)),
      }));
    ...
    return (
      <Card>
        <CardContent className="text-center py-16">
          ...
          <p className="text-muted-foreground mb-6">You can view and manage participants once events are available.</p>
        </CardContent>
      </Card>
    );
  }
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex-1 p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  {/* ✅ Only “View Participants” now */}
                  <div className="flex flex-col items-end gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        window.location.href = `/participants/${event.id}`;
                      }}
                    >
                      <Eye className="w-4 h-4 mr-2" /> View Participants
                    </Button>
                  </div>
                </div>
