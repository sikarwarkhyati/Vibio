...
const OrganizerDashboard: React.FC = () => {
  ...
  // Role-based access: allow only organizers and admins
  useEffect(() => {
    if (!authLoading && !user) {
      navigate('/auth');
    } else if (!authLoading && user) {
      ...
      const isOrganizer =
        userRole === 'organizer' ||
        userRole === 'organiser' ||
        (user as any)?.isOrganizer;
      const isAdmin =
        userRole === 'admin' || (user as any)?.isAdmin;
  ...
  const fetchOrganizerStats = async () => {
    ...
    try {
      const organizerId = (user as any)._id || (user as any).id;
      let res = await api
        .get(`/organizers/${organizerId}/stats`)
        .catch(() =>
          api.get('/organizers/stats', { params: { organizerId } })
        );
      ...
      setStats({
        totalEvents:
          ...
          payload.totalEventsCount ??
          (events?.length ?? 0),
        totalBookings:
          payload.totalBookings ?? payload.total_bookings ?? 0,
        revenue: payload.revenue ?? payload.totalRevenue ?? 0,
        avgRating:
          payload.avgRating ??
      ...
      toast({
        ...
        description:
          err.response?.data?.message ||
          err.message ||
          'Failed to fetch organizer stats',
        variant: 'destructive',
      });
    } finally {
      setStatsLoading(false);
  ...
  const isAdmin =
    role === 'admin' || (user as any)?.isAdmin;
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          >
                  ...
                  <CardContent className="space-y-4">
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground mb-4">
                        Create events, scan tickets, and monitor analytics.
                      </p>
                      <Button
                        onClick={() => setShowCreateForm(true)}
                        className="bg-gradient-to-r from-primary to-primary-glow hover:opacity-90"
                      >
                        <Plus className="w-4 h-4 mr-2" /> Create Your
                        First Event
                      </Button>
                    </div>
                  </CardContent>
            ...
            <TabsContent value="events">
              <MyEventsGrid organizerId={organizerId} />
            </TabsContent>
